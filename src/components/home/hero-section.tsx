  ...
  return (
    ...
    >
      <div className="home-hero-container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <div className="home-hero-layout grid grid-cols-1 lg:grid-cols-[1.3fr_300px] xl:grid-cols-[1.35fr_340px] gap-6 lg:gap-x-12 xl:gap-x-16 lg:gap-y-5 items-start">
            {/* 1. Header & Section Index */}
            <div className="hero-intro-header lg:col-start-1 lg:row-start-1 flex items-center gap-2.5 font-mono text-xs text-(--color-muted) border-b border-(--color-border)/70 pb-2.5">
            ...
            {/* 2. Identity Block: Greeting & Dominant Name */}
            <div className="hero-identity-block lg:col-start-1 lg:row-start-2 flex flex-col gap-1">
              <span className="font-mono text-xs sm:text-sm text-(--color-muted) uppercase tracking-wider font-medium">
                {copy.greeting}
              </span>
              <h1 className="home-hero-name font-serif text-4xl sm:text-5xl md:text-[3.5rem] lg:text-[4rem] xl:text-[4.75rem] font-normal tracking-tight text-(--color-foreground) leading-[1.02] m-0">
                {copy.name}
              </h1>
            </div>

            {/* 3. Role Introduction & Animated Role Reel */}
            <div className="hero-role-block lg:col-start-1 lg:row-start-3 flex flex-wrap items-center gap-2.5 sm:gap-3.5">
              <span
            ...
            {/* 4. Concise Approved Factual Bio (Readable measure ~65ch) */}
            <p className="home-hero-description lg:col-start-1 lg:row-start-4 font-sans text-sm sm:text-base text-(--color-muted) leading-relaxed max-w-[65ch] m-0">
              {copy.personalBrand}
            </p>

            {/* 5. Portrait Evidence Frame (After Bio on Mobile/Tablet; Right Column on Desktop) */}
            <div className="hero-portrait-col lg:col-start-2 lg:row-start-1 lg:row-span-6 flex justify-center lg:justify-end self-start my-2 lg:my-0">
            ...
            <div className="hero-meta-rail lg:col-start-1 lg:row-start-5 flex flex-wrap items-center gap-2.5 sm:gap-3.5 border-t border-(--color-border) pt-3 font-mono text-xs text-(--color-muted)">
            ...
            {/* 7. Action CTAs (Solid Primary & Borderless Secondary) */}
